...
pub async fn create_game(pool: &SqlitePool, game: &Game) -> Result<()> {
    sqlx::query(
        r#"
        INSERT INTO games (id, name, event_date, organizer_email, admin_token, created_at, drawn)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        "#,
    )
    .bind(&game.id)
...
pub async fn get_game_by_id(pool: &SqlitePool, game_id: &str) -> Result<Option<Game>> {
    let row = sqlx::query(
        r#"
...
pub async fn get_game_by_id_tx(
...
pub async fn get_game_by_admin_token(pool: &SqlitePool, admin_token: &str) -> Result<Option<Game>> {
    let row = sqlx::query(
        r#"
        SELECT id, name, event_date, organizer_email, admin_token, created_at, drawn
        FROM games
...
pub async fn add_participant(pool: &SqlitePool, participant: &Participant) -> Result<()> {
    sqlx::query(
        r#"
        INSERT INTO participants (id, game_id, name, email, matched_with_id, view_token, has_viewed, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        "#,
    )
    .bind(&participant.id)
...
pub async fn get_participants_by_game(
    pool: &SqlitePool,
    game_id: &str,
...
// Transaction-aware version
pub async fn get_participants_by_game_tx(
    tx: &mut sqlx::Transaction<'_, sqlx::Sqlite>,
    game_id: &str,
) -> Result<Vec<Participant>> {
    let rows = sqlx::query(
...
// Transaction-aware version
pub async fn update_participant_matches_tx(
    tx: &mut sqlx::Transaction<'_, sqlx::Sqlite>,
...
// Transaction-aware version
pub async fn mark_game_as_drawn_tx(
    tx: &mut sqlx::Transaction<'_, sqlx::Sqlite>,
    game_id: &str,
) -> Result<()> {
    sqlx::query(
...
pub async fn mark_participant_viewed(pool: &SqlitePool, participant_id: &str) -> Result<()> {
    sqlx::query(
        r#"
        UPDATE participants
        SET has_viewed = 1
...
pub async fn delete_game(pool: &SqlitePool, game_id: &str) -> Result<()> {
...
pub async fn cleanup_expired_verifications(pool: &SqlitePool) -> Result<u64> {
    let now = Utc::now();
...
pub async fn count_total_bulk_resends(
    pool: &SqlitePool,
    game_id: &str,
) -> Result<i64> {
    let row = sqlx::query(
...
pub async fn count_participants_in_game(pool: &SqlitePool, game_id: &str) -> Result<i64> {
    let row = sqlx::query(
        r#"
        SELECT COUNT(*) as count
        FROM participants
